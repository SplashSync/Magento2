...
class InvoiceItemsGetter
{
    ...
    public static function getItemsValues(Item $product, Currency $currency, string $fieldId)
    {
        switch ($fieldId) {
            ...
            case 'qty':
                return $product->getData("has_children") ? 0 : (int) $product->getQty();
            //====================================================================//
    ...
    private static function getDiscount(Item $orderItem)
    {
        ...
        if (($orderItem->getPriceInclTax() > 0) && ($orderItem->getQty() > 0)) {
            return (float) 100 * $orderItem->getDiscountAmount()
                / ($orderItem->getPriceInclTax() * $orderItem->getQty())
            ;
        }
    ...
    private static function getPrice(Item $orderItem, Currency $currency)
    {
        ...
        $ttcPrice = null;
        $itemTax = (float) $orderItem->getOrderItem()->getTaxPercent();
        //====================================================================//
        // Build Price Array
